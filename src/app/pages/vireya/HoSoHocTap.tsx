...
export default function HoSoHocTapPage() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      /*
       * Tải điểm tách biệt với danh mục.
       * Nếu subjectService/scoreTypeService gặp lỗi tạm thời,
       * dữ liệu điểm đã lưu vẫn được hiển thị thay vì bị che mất.
       */
      const loadResults = getAllLearningResults(userId)
        .then((resultData) => {
          if (!isMounted) return;
        ...
        .catch((error) => {
          ...
          if (isMounted) {
            toast.error("Không thể tải điểm đã lưu. Vui lòng thử lại.");
          }
        });
      ...
      const loadCatalogs = Promise.all([
        ...
        .then(([subjectData, scoreTypeData]) => {
          if (!isMounted) return;
          setSubjects(Array.isArray(subjectData) ? subjectData : []);
          setScoreTypes(Array.isArray(scoreTypeData) ? scoreTypeData : []);
        })
        .catch((error) => {
          console.error("Lỗi tải danh mục môn học/loại điểm:", error);
          if (isMounted) {
            toast.warning("Không thể tải đầy đủ danh mục hiển thị.");
          }
        });

      await Promise.all([loadResults, loadCatalogs]);
  ...
  const handleSave = async () => {
    if (!editingKey || !userId || saving) return;
    ...
    const syncResultsFromDatabase = async () => {
      const latestResults = await getAllLearningResults(userId);
    ...
    try {
      ...
      /*
       * Không thay đổi cách lưu trữ hiện tại:
       * vẫn xóa/cập nhật đúng các document learningResults đang có.
       */
      ...
      /*
       * Đọc lại từ Firestore sau khi ghi thay vì chỉ tự sửa state cục bộ.
       * Nhờ đó dữ liệu màn hình khớp dữ liệu thật sau refresh/mạng chập chờn.
       */
      await syncResultsFromDatabase();
      ...
      /*
       * Nếu một thao tác đã ghi thành công trước khi thao tác khác lỗi,
       * tải lại dữ liệu thật để tránh màn hình hiển thị sai.
       */
      try {
        await syncResultsFromDatabase();
      } catch (reloadError) {
        console.error("Lỗi tải lại dữ liệu sau khi lưu thất bại:", reloadError);
      }

      resetEditState();
      toast.error(
        "Có lỗi khi lưu. Dữ liệu hiển thị đã được đồng bộ lại theo hệ thống."
      );
    } finally {
      setSaving(false);
    }
